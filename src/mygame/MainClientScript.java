...
import CliientMap.BlockInformationClient;
...
public class MainClientScript extends SimpleApplication implements ActionListener {
    ...
    public void displayParts(BlockInformationClient blockInformation) {
        ...
        for (int i = 0; i < blockInformation.getSize(); i++) {
            ...
            if (!loadedParts.containsKey(weltElementID)) {
                ...
                ClientMapSpatial mapSpatial = new ClientMapSpatial(map.map[0].welt.worldBlocks.weltElemente[x][y][z], terrain, landscapeControl);
                loadedParts.put(weltElementID, mapSpatial);
                if (elementNumber != 2) {
    ...
    HashMap<String, ClientMapSpatial> loadedParts;
    ...
    public void simpleInitApp() {
        ...
        loadedParts = new HashMap<String, ClientMapSpatial>();
        map = new ClientMap();
    ...
    private void initClient() {
        //"127.0.0.1"
        client = new Client("192.168.2.109", 500, 4);
        client.startClient();
        client.startNewConnection("127.0.0.1", 500);
    }
...
//Player moveMent
        ...
        if (left) {
            if (run) {
                walkDirection.addLocal(camLeft).mult(5f);
            } else {
                walkDirection.addLocal(camLeft).mult(0.2f);
            }
        } else if (right) {
            if (run) {
                walkDirection.addLocal(camLeft.negate().mult(5f));
            } else {
            ...
            if (run) {
                walkDirection.addLocal(camDir.negate().mult(5f));
            } else {
